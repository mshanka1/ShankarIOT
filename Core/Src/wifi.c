...
void wifi_sendCommand(uint8_t *command,int size){
...
void sendData(){
	 wifi_sendCommand((uint8_t*)STARTTCPcommand, strlen(STARTTCPcommand));
	 ...
	 int payloadsize=strlen(POSTmid)+strlen(payloadsensors)+strlen(POSTclose)-4;
	  ...
	  wifi_sendCommand(DATAsend,strlen(DATAsend));
	  wifi_sendCommand((uint8_t*)CLOSETCPcommand, strlen(CLOSETCPcommand));
	  sleep_thx(TIME_INTERVAL);
	  indexdata=0;
	  indexsensor=0;
}
...
void startWifi(){
	  ...
	  wifi_sendCommand((uint8_t*) SSIDcommand,strlen(SSIDcommand));
	  wifi_sendCommand((uint8_t*) PSWcommand,strlen(PSWcommand));
	  wifi_sendCommand((uint8_t*) SECcommad,strlen(SECcommad));
	  wifi_sendCommand((uint8_t*) CONNECTcommand,strlen(CONNECTcommand));
	  wifi_sendCommand((uint8_t*)TCPcommand,strlen(TCPcommand));
	  wifi_sendCommand((uint8_t*)IPcommand, strlen(IPcommand));
	  wifi_sendCommand((uint8_t*)PORTcommand, strlen(PORTcommand));
}
