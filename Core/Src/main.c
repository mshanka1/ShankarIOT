...
extern TX_THREAD tx_app_sensor_thread;
extern TX_THREAD tx_app_wifi_thread;
...
void tx_app_wifi_thread_entry(ULONG thread_input)
{
	while(1)
	{
		tx_thread_sleep(200);
...
void tx_app_sensor_thread_entry(ULONG thread_input)
{
	while(1)
	{
		tx_thread_sleep(200);
...
void tx_app_thread_entry(ULONG thread_input)
{
  /* USER CODE BEGIN tx_app_thread_entry */
	...
	if((local==0)&&(thread_flag == 0))
	{
	  ...
	  local =1;
	  thread_flag = 1;
	  tx_thread_resume(&tx_app_wifi_thread);
	  tx_thread_resume(&tx_app_sensor_thread);
	}
	sleep_thx(1000);
	  //WIFI
	  //HAL_GPIO_WritePin(WIFI_RESET_GPIO_Port,WIFI_RESET_Pin,GPIO_PIN_RESET);
