...
/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */
extern void sleep_thx(uint32_t);
/* USER CODE END ET */
...
/* USER CODE BEGIN EFP */
struct T_SENSOR_UNITS_MANAGEMENT
{
	uint32_t flag;
	int16_t distanceComplete;
	float press;
	float hum;
	float temp;
	...
	int16_t prev_distanceComplete;
	float prev_press;
	float prev_hum;
	float prev_temp;
	int16_t prev_accx,prev_accy,prev_accz;
	int16_t prev_magx,prev_magy,prev_magz;
};
